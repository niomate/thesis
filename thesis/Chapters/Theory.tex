...
\end{equation}
So for the PSNR, a larger value is a better one unlike the MSE, where 0 is the best value one can
possibly reach.
...
\end{equation} 
as proposed by Rohr, 1991\cite{rohr91} or the determinant 
\begin{equation}
    \det(\mathbf{J}_\rho) := j_{1, 1}j_{2, 2} -
    j_{1, 2}^2 = \lambda_1\lambda_2\label{def:Harris}
\end{equation} 
as proposed by Harris\cite{harris88} and
Förstner\cite{foerstner87}, 1988 and 1987
respectively. Both of these approaches do not need to explicitly compute the eigenvalues of
the structure tensor and are thus not as computationally invested. Another difference between both 
approaches is that~\eqref{def:Rohr} requires 
\begin{equation}
    \tr(\mathbf{J}_\rho)
\end{equation}
...
%\begin{align}
    %&\partial_t u = \textnormal{div}(\boldsymbol D\del u)&\textnormal{on}\ 
    %\Omega\times[0, \infty)\\
    %&u(\boldsymbol x, 0) = f(\boldsymbol x)&\textnormal{on}\ \Omega\\
    %&\boldsymbol n^\top \boldsymbol D\del u = 0&\textnormal{on}\ 
...
\begin{equation}
    \partial_t u = \diverg(g(\del u_\sigma\del u_\sigma^\bot)\del u)\label{eq:EED}
\end{equation}
...
%\begin{figure}[h]
    %\centering
    %\includegraphics[width=\linewidth]{../Images/inpainting_bertalmio.png}
    %\caption{Example for inpainting based on prolongation of isophote lines. Source: \cite{bertalmio00}}
    %\label{fig:InpaintingExample}
%\end{figure}
...
research, but it was shown in~\cite{schmaltz14}, that edge-enhancing diffusion~\eqref{eq:EED} is the best choice
among those tested.
