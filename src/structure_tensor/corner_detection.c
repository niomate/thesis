...
(float ***v,  /* image !! fgets smoothed on exit !! */
...

{
    ...
    for (m = 0; m <= nc - 1; m++) {
        ...
        dummies_mirror (v[m], nx, ny);

        for (i = 1; i <= nx; i++)
            for (j = 1; j <= ny; j++) {
                /* modified Sobel derivatives */
                dv_dx =
                h1 * (v[m][i + 1][j] - v[m][i - 1][j]) +
                h2 * (v[m][i + 1][j - 1] - v[m][i - 1][j - 1] + v[m][i + 1][j + 1] - v[m][i - 1][j + 1]);
                dv_dy =
                h3 * (v[m][i][j + 1] - v[m][i][j - 1]) +
                h4 * (v[m][i - 1][j + 1] - v[m][i - 1][j - 1] + v[m][i + 1][j + 1] - v[m][i + 1][j - 1]);
                /* tensor product */
