...

{
  ...
  for (i = 1; i <= nx; i++)
    for (j = 1; j <= ny; j++) {
      ...
      woo[i][j] = -wpo[i][j] - wop[i][j] - wpp[i][j] - wpm[i][j] - wmo[i][j] -
                  wom[i][j] - wmm[i][j] - wmp[i][j];
    }
...

{
  ...
  for (i = 1; i <= nx; i++)
    for (j = 1; j <= ny; j++) {
      help =
          f[i][j] - (boo[i][j] * u[i][j] + bpo[i][j] * u[i + 1][j] +
                     bmo[i][j] * u[i - 1][j] + bop[i][j] * u[i][j + 1] +
                     bom[i][j] * u[i][j - 1] + bpp[i][j] * u[i + 1][j + 1] +
                     bmm[i][j] * u[i - 1][j - 1] + bpm[i][j] * u[i + 1][j - 1] +
                     bmp[i][j] * u[i - 1][j + 1]);
      r = r + help * help;
    }
...

{
  ...
  for (i = 1; i <= nx; i++)
    for (j = 1; j <= ny; j++)
      u[i][j] = (boo[i][j] * f[i][j] + bpo[i][j] * f[i + 1][j] +
                 bmo[i][j] * f[i - 1][j] + bop[i][j] * f[i][j + 1] +
                 bom[i][j] * f[i][j - 1] + bpp[i][j] * f[i + 1][j + 1] +
...

{
  ...
  *k = 0;
...

  {
    ...
    /* compute q = A * p */
    matrix_times_vector(nx, ny, boo, bpo, bmo, bop, bom, bpp, bmm, bpm, bmp, p,
                        q);
...

{
  ...
  weights(dxx, dxy, dyy, a, nx, ny, hx, hy, alpha, gamma, woo, wpo, wmo, wop,
          wom, wpp, wmm, wpm, wmp);
  ...
  for (i = 1; i <= nx; i++)
    for (j = 1; j <= ny; j++)
      u[i][j] = f[i][j] +
                ht * (woo[i][j] * f[i][j] + wpo[i][j] * f[i + 1][j] +
                      wop[i][j] * f[i][j + 1] + wpp[i][j] * f[i + 1][j + 1] +
                      wpm[i][j] * f[i + 1][j - 1] + wmo[i][j] * f[i - 1][j] +
                      wom[i][j] * f[i][j - 1] + wmm[i][j] * f[i - 1][j - 1] +
                      wmp[i][j] * f[i - 1][j + 1]);
...

{
  ...
  for (i = 1; i <= nx; i++)
    for (j = 1; j <= ny; j++)
      u[i][j] = woo[i][j] * f[i][j] + wpo[i][j] * f[i + 1][j] +
                wmo[i][j] * f[i - 1][j] + wop[i][j] * f[i][j + 1] +
                wom[i][j] * f[i][j - 1] + wpp[i][j] * f[i + 1][j + 1] +
  ...
  if (stype == 0)
    /* conjugate gradients */
    CG(imax, nx, ny, woo, wpo, wmo, wop, wom, wpp, wmm, wpm, wmp, u, &*count,
       v);
  else if (stype == 1)
    /* Gauss-Seidel */
    GS(imax, nx, ny, woo, wpo, wmo, wop, wom, wpp, wmm, wpm, wmp, u, &*count,
       v);
...
void inpainting(char *in, char *mask, char *out, long dtype, float lambda,
                float sigma, float rho, float alpha, float gamma,
                long timediscr, float ht, long kmax, long stype, long imax)

{
  ...
  res = elliptic_residue(nx, ny, hx, hy, dtype, lambda, sigma, rho, alpha,
                         gamma, a, f);
  ...
  while (k < kmax) {
    ...
    if (timediscr == 0)
      ...
      eed_im(ht, nx, ny, hx, hy, dtype, lambda, sigma, rho, alpha, gamma, stype,
             imax, a, &count, f);
...
*/
void inpainting_compression(char *in, char *mask, char *out, float lambda,
                            float sigma, float alpha, float gamma,
                            long outer_iter, long solver_iter) {
  inpainting(in, mask, out, 0, lambda, sigma, 0.0, alpha, gamma, 1, 1000,
             outer_iter, 0, solver_iter);
}
...

{
  ...
  /* Initialise initial image to constant value outside of the mask */ 
  for (i = 1; i <= nx; ++i) {
    for (j = 1; j <= ny; ++j) {
      if (a[i][j] <= 0.00001) {
        f[i][j] = 128;
