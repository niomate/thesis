    ...
    for (i = 1; i <= nx; i++) {
        for (j = 1; j <= ny; j++) {
            ...
            dv_dx = h1 * (v[i + 1][j] - v[i - 1][j])
                + h2 * (v[i + 1][j - 1] - v[i - 1][j - 1] + v[i + 1][j + 1] - v[i - 1][j + 1]);
            dv_dy = h3 * (v[i][j + 1] - v[i][j - 1])
                + h4 * (v[i - 1][j + 1] - v[i - 1][j - 1] + v[i + 1][j + 1] - v[i + 1][j - 1]);
            /* tensor product */
            dxx[i][j] = dv_dx * dv_dx;
            dxy[i][j] = dv_dx * dv_dy;
            dyy[i][j] = dv_dy * dv_dy;
...

{
    ...
    printf("Computing corner response.\n");
    if (type == 0)
        /* Rohr: det(J) */
        for (i = 1; i <= nx; i++) {
            for (j = 1; j <= ny; j++) {
    ...
    if (type == 3) {
        /* Harrison measure det(J) - kappa * trace(J)^2 */
        for (i = 1; i <= nx; i++) {
            for (j = 1; j <= ny; j++) {
                trace = dxx[i][j] + dyy[i][j];
                det = dxx[i][j] * dyy[i][j] - dxy[i][j] * dxy[i][j];
...

{
    ...
    char* out = "output.pgm";          /* filename output image */
    float** u;          /* input image */
    ...
    /* corner detection */
    corner_detection(u, nx, ny, 1.0, 1.0, type, T, sigma, rho, kappa, v);
    ...
    if (type == 0) {
        ...
        comment_line(comments, "# Foerstner-Harris corner detector (Harmonic mean)\n");
    } else if (type == 3) {
        comment_line(comments, "# Harris measure\n");
    }
