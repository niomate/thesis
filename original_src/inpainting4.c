...
for (i=1; i<=nx; i++)
 for (j=1; j<=ny; j++)
     ...
     wpp[i][j] = alpha * rxx * dxx[i][j]
                 + alpha * ryy * dyy[i][j]
                 + (beta[i][j] + 1.0) * rxy * dxy[i][j]; 

     wpm[i][j] = alpha * rxx * dxx[i][j-1]
                 + alpha * ryy * dyy[i][j-1]
...
for (i=1; i<=nx; i++)
 for (j=1; j<=ny; j++)
     ...
     woo[i][j] = - wpo[i][j] - wop[i][j] - wpp[i][j] - wpm[i][j]
                 - wmo[i][j] - wom[i][j] - wmm[i][j] - wmp[i][j];
     }     
...
for (i=1; i<=nx; i++)
 for (j=1; j<=ny; j++)
     u[i][j] = ( boo[i][j] * f[i][j]  
               ...
               + bpp[i][j] * f[i+1][j+1] + bmm[i][j] * f[i-1][j-1]
               + bpm[i][j] * f[i+1][j-1] + bmp[i][j] * f[i-1][j+1] );
...
while ((eps >= 0.000000000000000001 * rho) && (*k < kmax))
      ...
      /* compute q = A * p */
      matrix_times_vector (nx, ny, boo, bpo, bmo, bop, bom, bpp, bmm, 
                           bpm, bmp, p, q);
...
weights (dxx, dxy, dyy, a, nx, ny, hx, hy, alpha, gamma, 
...
for (i=1; i<=nx; i++)
 for (j=1; j<=ny; j++)
     u[i][j] =   woo[i][j] * f[i][j]  
               + wpo[i][j] * f[i+1][j]   + wmo[i][j] * f[i-1][j]
...
if (stype == 0)
   /* conjugate gradients */
   CG (imax, nx, ny, woo, wpo, wmo, wop, wom, wpp, wmm, wpm, wmp, u, 
      &*count, v); 
else if (stype == 1)
   /* Gauss-Seidel */
   GS (imax, nx, ny, woo, wpo, wmo, wop, wom, wpp, wmm, wpm, wmp, u, 
      &*count, v); 
...
res = elliptic_residue (nx, ny, hx, hy, dtype, lambda, sigma, rho,
                        alpha, gamma, a, f);
...
while (k < kmax)
      ...
      if (timediscr == 0)
         /* explicit scheme */
         eed_ex (ht, nx, ny, hx, hy, dtype, lambda, sigma, rho, alpha, 
                 gamma, a, f);
      else if (timediscr == 1)   
         /* semi-implicit scheme */
         eed_im (ht, nx, ny, hx, hy, dtype, lambda, sigma, rho, alpha, 
                 gamma, stype, imax, a, &count, f);

      /* check stable pixels and other things every 10th iteration */
      if (10 * (k/10) == k)
         { 
         analyse (f, nx, ny, &min, &max, &mean, &stdev, &norm);
         res = elliptic_residue (nx, ny, hx, hy, dtype, lambda, sigma, 
                                 rho, alpha, gamma, a, f);
